...
import co.actioniq.slick.dao.{DAO, DAODynamicProfileTable, DAOLongIdQuery, DbLongOptId, FormValidatorMessageSeq, IdModel, JdbcTypeImplicits, JdbcTypes}
...
class DynamicTypesSpec extends Specification with Mockito {
  ...
  abstract class PersistedDao[P <: JdbcProfile](
    ...
    override protected def addUpdateTransaction(id: DbLongOptId, input: DynamicSlick, original: DynamicSlick): Unit = ???

    override protected def addDeleteTransaction(id: DbLongOptId, original: DynamicSlick): Unit = ???

    override def nameSingle: String = ???

    override def validateCreate(input: DynamicSlick)(implicit ec: ExecutionContext): DBIOAction[FormValidatorMessageSeq, NoStream, Effect.Read] = ???

    override def validateUpdate(input: DynamicSlick, original: DynamicSlick)(implicit ec: ExecutionContext): DBIOAction[FormValidatorMessageSeq, NoStream, Effect.Read] = ???
  }
