...
import co.actioniq.slick.dao.{DAOLongIdQuery, DbLongOptId, FormValidatorMessageSeq, H2DAO, H2DAOTable, IdModel, JdbcTypeImplicits, JdbcTypes}
import co.actioniq.slick.logging.{NoopBackend, TransactionLogger}
...
class AiqSlickProfileSpec extends Specification with Mockito {
  ...
  class PersistedDao(
    ...
    override protected def addUpdateTransaction(id: DbLongOptId, input: PersistedSlick, original: PersistedSlick): Unit = ???

    override protected def addDeleteTransaction(id: DbLongOptId, original: PersistedSlick): Unit = ???

    override def nameSingle: String = ???

    override def validateCreate(input: PersistedSlick)(implicit ec: ExecutionContext): profile.api.DBIOAction[FormValidatorMessageSeq, profile.api.NoStream, Effect.Read] = ???

    override def validateUpdate(input: PersistedSlick, original: PersistedSlick)(implicit ec: ExecutionContext): profile.api.DBIOAction[FormValidatorMessageSeq, profile.api.NoStream, Effect.Read] = ???
  }
}
